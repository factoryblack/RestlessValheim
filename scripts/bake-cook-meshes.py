# Import Meshy FBX in Blender and write a Unity-Y-up binary mesh (.rcm).
# The 4K master stays in the Meshy zip. 1024 albedo is copied beside the .rcm.
import shutil
import struct
...
src_dir = root / "art" / "cook" / "mesh"
out_dir = root / "art" / "cook" / "runtime"
    ...
    for obj in list(bpy.data.objects):
        ...
        for tri in mesh.loop_triangles:
            ...
            for loop_i in tri.loops:
                ...
                # Meshy FBX UVs are already Unity-oriented. Do not flip V.
                verts.append((pos.x, pos.y, pos.z, nrm.x, nrm.y, nrm.z, uv.x, uv.y))
                tri_idx.append(len(verts) - 1)
            # Blender is CCW; Unity wants CW after the Z-up to Y-up remap.
...
def main():
    ...
    if len(files) != 35:
        raise SystemExit(f"expected 35 fbx, found {len(files)}")
