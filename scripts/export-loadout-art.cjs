...
// npm install sharp; node scripts/export-loadout-art.cjs
...
const names = [
  ['loadout-corner', 256],
  ['loadout-crest', 128],
  ['loadout-stat-plaque', 768],
  ['equipment-set-seal', 128]
];
(async () => {
  ...
  for (const [name, width] of names) {
    const source = path.join(root, 'art/loadout/originals', name + '.png');
  ...
  fs.writeFileSync(path.join(root, 'art/loadout/manifest.json'), JSON.stringify(manifest, null, 2) + '\n');
